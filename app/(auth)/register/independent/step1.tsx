...
export default function IndependentStep1() {
  ...
  return (
    <ScreenWrapper scroll className="px-6">
      ...
      <Text className="text-primary text-2xl font-heading mb-6">{t('registration.personalInfo')}</Text>
      {(['fullName', 'dateOfBirth', 'phone', 'email', 'password', 'address', 'city', 'state', 'zip'] as const).map((field) => (
        <Controller key={field} control={control} name={field} render={({ field: { onChange, value } }) => (
          <Input
            label={t(`registration.${field === 'fullName' ? 'fullName' : field === 'dateOfBirth' ? 'dateOfBirth' : field}`)}
            value={value}
