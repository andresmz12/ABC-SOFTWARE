    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.background }}>
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24 }}>
        ...
        <View style={{ paddingTop: 8, paddingBottom: 24 }}>
          ...
          <Text style={{ color: C.textMuted, fontSize: 14, fontFamily: 'Inter_400Regular', marginTop: 6 }}>What services do you offer and where?</Text>
        </View>
        ...
        {selected.length > 0 && (
          <View style={{ backgroundColor: C.surface, borderWidth: 1, borderColor: C.line, borderRadius: 14, padding: 16, marginBottom: 24 }}>
            <Text style={{ color: C.textMuted, fontSize: 12, fontFamily: 'Inter_400Regular', marginBottom: 8 }}>{selected.length} area{selected.length > 1 ? 's' : ''} selected</Text>
        ...
        <Button label="Continue" onPress={() => router.push('/(auth)/register/company/step3' as any)} disabled={selected.length === 0} />
