...
export default function CompanyStep2() {
  ...
  const US_STATES = ['Alabama','Alaska','Arizona','California','Colorado','Florida','Georgia','Illinois','New York','Texas','Virginia','Washington'];
    ...
    <ScreenWrapper scroll className="px-6">
      ...
      <View className="flex-row flex-wrap gap-2 mb-6">
        {US_STATES.map((state) => (
          ...
          >
            <Text className={`text-xs ${selectedStates.includes(state) ? 'text-white font-body-medium' : 'text-text-muted'}`}>{state}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <Button
        label={t('common.next')}
        onPress={() => router.push('/(auth)/register/company/step3' as any)}
