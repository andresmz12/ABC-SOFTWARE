...
export default function ProviderPayments() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[ProviderPayments]', e.message);
    } finally {
      setLoading(false);
    }
  }, [user?.id]);
  ...
  const renderItem = ({ item }: { item: Payment }) => {
    ...
    return (
      <View style={{ backgroundColor: C.surface, borderRadius: 14, borderWidth: 1, borderColor: C.line, padding: 14, marginBottom: 10, flexDirection: 'row', alignItems: 'center' }}>
        <View style={{ width: 42, height: 42, borderRadius: 12, backgroundColor: `${C.success}18`, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
        ...
        <View style={{ alignItems: 'flex-end', gap: 4 }}>
          <Text style={{ color: C.success, fontSize: 15, fontFamily: 'Inter_700Bold' }}>{amountText}</Text>
        ...
        {payments.filter((p) => p.status === 'paid').length > 0 && (
          <View style={{ marginLeft: 32, backgroundColor: `${C.success}15`, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 6, alignSelf: 'flex-start', borderWidth: 1, borderColor: `${C.success}30` }}>
            <Text style={{ color: C.success, fontSize: 13, fontFamily: 'Inter_600SemiBold' }}>
              {es ? 'Total ganado' : 'Total earned'}: {isColombia ? formatCOP(totalEarned) : formatUSD(totalEarned)}
            </Text>
          </View>
        )}
      </View>
