...
export default function JobCard({ job, onPress }: Props) {
  ...
  const budgetText = job.budget_usd
    ? job.budget_max_usd ? `$${job.budget_usd}–$${job.budget_max_usd}` : `$${job.budget_usd}`
    : job.budget_cop
    ? job.budget_max_cop ? `${job.budget_cop.toLocaleString()}–${job.budget_max_cop.toLocaleString()} COP` : `${job.budget_cop.toLocaleString()} COP`
    : null;
    ...
    >
      <View className="p-4">
        ...
        <View className="flex-row items-center gap-2 mb-3">
          ...
          <Text className="text-text-muted font-body text-xs">📍 {job.city}, {job.state}</Text>
        </View>
      ...
      <View className="bg-primary px-4 py-3 items-center">
        <Text className="text-white font-body-bold text-sm tracking-wide">Apply Now</Text>
      </View>
    </TouchableOpacity>
  );
}
